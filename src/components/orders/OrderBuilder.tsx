...
export default function OrderBuilder({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 xl:gap-8">
              ...
              <li key={p.id} className="py-3 flex items-center gap-3">
                ...
                {selected ? (
                  ...
                  <button
                    type="button"
                    onClick={() => inc(p)}
                    className="px-3 py-1.5 text-xs uppercase tracking-eyebrow border border-gold/40 rounded text-cream hover:border-gold/70"
                  >
                    Agregar
                  </button>
                )}
                {selected && (
                  <button
                    ...
                    onClick={() => dec(p)}
                    aria-label="Quitar"
                    className="w-8 h-8 flex items-center justify-center border border-wine/40 rounded text-wine hover:bg-wine/10"
                  >
                    ×
                  </button>
                )}
      ...
      <aside className="lg:sticky lg:top-24 h-fit space-y-4 bg-surface-1 border border-gold/15 rounded-lg p-5">
        <div>
